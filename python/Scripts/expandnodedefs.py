...
def generateMarkdownTable(ports, headers, nodedefName=None):
    '''Generate a markdown table from a list of ExpandedPort objects.'''
    # Build headers, optionally adding NodeDef column on the left
    tableHeaders = list(headers)
    if nodedefName is not None:
        tableHeaders.insert(0, nodedefName)

    # First pass: collect all cell values for all rows
    allRows = []
    for port in ports:
        cells = []
        # Add empty cell for NodeDef column (nodedef name is in header)
        if nodedefName is not None:
            cells.append('')
        for h in headers:
            hLower = h.lower()
    ...
    colWidths = []
    for colIdx, header in enumerate(tableHeaders):
        maxWidth = len(header)
        for row in allRows:
            if colIdx < len(row):
    ...
    headerCells = [h.ljust(colWidths[i]) for i, h in enumerate(tableHeaders)]
    ...
    for row in allRows:
        paddedCells = [row[i].ljust(colWidths[i]) for i in range(len(tableHeaders))]
    ...
    for nodeName, section in parsedSpec.nodeSections.items():
        for table in section.tables:
            ...
            for i, (sig, ports, nodedefName) in enumerate(expanded):
                ...
                tableLines = generateMarkdownTable(ports, table.headers, nodedefName)
...
def main():
    ...
    parser.add_argument('--mtlx', dest='mtlxFile',
        help='Path to the data library MaterialX document. If provided, adds NodeDef column with matching nodedef names.')
