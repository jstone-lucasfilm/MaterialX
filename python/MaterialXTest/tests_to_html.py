...
def computeDiff(image1Path, image2Path, imageDiffPath):
    try:
        ...
        return float(sum(diffStat.rms)) / float(len(diffStat.rms))
    except Exception as err:
        print ("Failed to compute difference image: " + err)
        if os.path.exists(imageDiffPath):
            os.remove(imageDiffPath)
    ...
    if langFiles1:
        ...
        for file1, file2, file3, path1, path2, path3 in zip(langFiles1, langFiles2, langFiles3, langPaths1, langPaths2, langPaths3):
            ...
            if fullPath3:
                ...
                fh.write("</td>\n")
            if diffPath1:
                fh.write("<td align='center'>Difference " + args.lang1 + "/" + args.lang2 + " (RMS " + "%.2f" % diffRms1 + ") </td>\n")
            if diffPath2:
                fh.write("<td align='center'>Difference " + args.lang1 + "/" + args.lang3 + " (RMS " + "%.2f" % diffRms2 + ") </td>\n")
            if diffPath3:
                fh.write("<td align='center'>Difference " + args.lang2 + "/" + args.lang3 + " (RMS " + "%.2f" % diffRms3 + ") </td>\n")
            fh.write("</tr>\n")
