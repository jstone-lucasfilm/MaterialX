...
The `tint` parameter colors the reflected and transmitted light but should be left at white (1,1,1) for physically correct results. Setting the `ior` to 0.0 disables the Fresnel curve, allowing reflectivity to be controlled purely by weight and tint.
...
|`roughness`         |Surface roughness along the tangent and bitangent              |vector2|0.05, 0.05   |               |
...
|`roughness`         |Surface roughness                                        |vector2|0.05, 0.05            |               |
|`thinfilm_thickness`|Thickness of the iridescent thin-film layer in nanometers|float  |0.0                   |               |
|`thinfilm_ior`      |Index of refraction of the thin-film layer               |float  |1.5                   |               |
|`normal`            |Normal vector of the surface                             |vector3|Nworld                |               |
|`tangent`           |Tangent vector of the surface                            |vector3|Tworld                |               |
...
The `color82` parameter provides a multiplier on reflectivity at 82 degrees, useful for capturing the characteristic "dip" in the reflectance curve of metallic surfaces. Setting it to (1,1,1) effectively disables this feature for backward compatibility.
...
|`out`   |Output: the computed EDF                      |EDF    |__empty__    |
...
|`color`      |Radiant emittance of light leaving the surface          |color3        |1.0, 1.0, 1.0        |
|`normal`     |Normal vector of the surface                            |vector3       |Nworld               |
|`inner_angle`|Angle of inner cone where intensity falloff starts      |uniform float |60.0                 |
|`outer_angle`|Angle of outer cone where intensity goes to zero        |uniform float |0.0                  |
...
|`color` |Radiant emittance of light leaving the surface    |color3           |1.0, 1.0, 1.0        |
|`normal`|Normal vector of the surface                      |vector3          |Nworld               |
|`file`  |Path to a file containing IES light profile data  |uniform filename |__empty__            |
|`out`   |Output: the computed EDF                          |EDF              |                     |
...
|`base`    |The base EDF to be modified                                  |EDF    |             |
|`out`     |Output: the computed EDF                                     |EDF    |             |
...
|Port        |Description                                                                                                                                       |Type   |Default      |
|------------|--------------------------------------------------------------------------------------------------------------------------------------------------|-------|-------------|
|`absorption`|Absorption rate for the medium (rate per distance traveled in the medium, given in _m<sup>−1</sup>_). Set for each color component/wavelength separately|color3 |0.0, 0.0, 0.0|
|`out`       |Output: the computed VDF                                                                                                                          |VDF    |__empty__    |
...
The `anisotropy` parameter controls the scattering direction: negative values produce backwards scattering, positive values produce forward scattering, and 0.0 produces uniform scattering. Both absorption and scattering rates are specified per color channel/wavelength.

|Port        |Description                                                                                        |Type   |Default      |Accepted Values|
|------------|---------------------------------------------------------------------------------------------------|-------|-------------|---------------|
|`absorption`|Absorption rate for the medium (in _m<sup>−1</sup>_)                                               |color3 |0.0, 0.0, 0.0|               |
|`scattering`|Scattering rate for the medium (in _m<sup>−1</sup>_)                                               |color3 |0.0, 0.0, 0.0|               |
|`anisotropy`|Anisotropy factor for scattering direction                                                         |float  |0.0          |[-1, 1]        |
...
|Port         |Description                                         |Type          |Default   |
|-------------|----------------------------------------------------|--------------|----------|
|`bsdf`       |Bidirectional scattering distribution function      |BSDF          |          |
|`edf`        |Emission distribution function for the surface      |EDF           |          |
...
|Port  |Description                                          |Type         |Default   |
|------|-----------------------------------------------------|-------------|----------|
|`vdf` |Volume distribution function for the medium          |VDF          |          |
|`edf` |Emission distribution function for the medium        |EDF          |          |
...
|Port  |Description                                |Type                |Default   |Accepted Values|
|------|-------------------------------------------|--------------------|----------|---------------|
|`bg`  |The first distribution function            |BSDF, EDF, or VDF   |__empty__ |               |
|`fg`  |The second distribution function           |Same as `bg`        |__empty__ |               |
|`mix` |The mixing weight                          |float               |0.0       |[0, 1]         |
|`out` |Output: the mixed distribution function    |Same as `bg`        |__empty__ |               |
...
|Port  |Description                     |Type         |Default   |
|------|--------------------------------|-------------|----------|
|`top` |The top BSDF                    |BSDF         |          |
|`base`|The base BSDF or VDF            |BSDF or VDF  |          |
...
|Port  |Description                                |Type                |Default   |
|------|-------------------------------------------|--------------------|----------|
|`in1` |The first distribution function            |BSDF, EDF, or VDF   |          |
|`in2` |The second distribution function           |Same as `in1`       |          |
...
|Port  |Description                              |Type                |Default   |
|------|-----------------------------------------|--------------------|----------|
|`in1` |The distribution function to scale       |BSDF, EDF, or VDF   |          |
...
|`out`       |Output: the computed roughness    |vector2 |__zero__|               |
...
|`anisotropy`|Amount of anisotropy              |float   |0.0     |[0, 1]         |
|`out`       |Output: the computed roughness    |vector2 |0.0     |               |
