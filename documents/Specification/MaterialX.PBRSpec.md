...
The models defined here are the [Autodesk Standard Surface](#autodesk-standard-surface), [UsdPreviewSurface](#usdpreviewsurface), [Khronos glTF PBR](#khronos-gltf-pbr), and [OpenPBR Surface](#openpbr-surface), ranging from an intentionally compact preview model to full über-surfaces. Each model is presented as a table of its inputs followed by a functional definition in the [Shading Model Notation](#shading-model-notation) below, with a link to the corresponding renderable nodegraph in the MaterialX data libraries.

All of these models are also specified upstream by their originating authors and organizations, and those specifications are where the physical and perceptual foundations of each model live. Because their purpose is to convey a model's design and intent, they often, by design, provide flexibility in how it is implemented.

MaterialX addresses a complementary need: producing consistent visuals across the many applications and renderers that support it requires each model to have a single, unambiguous definition. The definitions in this section provide that. Each one documents the MaterialX realization of its model — the exact composition rendered by its data-library nodegraph, concretizing choices the upstream specification leaves open — and notes the version of the model that it documents, together with any points where the composition differs from the upstream model. The two documents are meant to be read together: the upstream specification remains authoritative for the intent of the model, and this section for its MaterialX rendering.


## Shading Model Notation

The shading models in this section are defined by a compact functional notation that follows the structure of each model's MaterialX nodegraph, so the flow from artist-facing inputs through the PBS nodes to a final shader can be read directly from the document.

Each model is presented as a list of its inputs — the **interface** of the corresponding node — followed by a function that takes those inputs and returns a `surfaceshader`:
...
* **Parameters.** Each parameter is one of the model's inputs, matching an `<input>` of its `<nodedef>`; defaults and accepted values are given in the interface table rather than repeated here. Inputs the composition does not implement are omitted, as noted per model. `Nworld` and `Tworld` denote the default world-space geometric normal and tangent (the `defaultgeomprop` of the corresponding input).
...
* **Operators.** The operators `+`, `-`, `*`, `/`, and `^` denote the `<add>`, `<subtract>`, `<multiply>`, `<divide>`, and `<power>` nodes, following conventional precedence: `^` binds most tightly, then `*` and `/`, then `+` and `-`, with parentheses grouping explicitly. When one operand is a distribution function, `*` scales it by a `float` or `color3`.

* **Mixing.** `mix(fg, bg, w)` is the `<mix>` node: it returns `fg` at `w` = 1 and `bg` at `w` = 0, equal to (1 − `w`)·`bg` + `w`·`fg`.

* **Conditionals.** `if (cond) { a } else { b }` selects between two values of the same type — the type of the result — where `cond` is a `boolean` or a comparison using `>`, `>=`, or `==`. A boolean maps to an `<ifequal>` test against `true` (or a `<switch>`); the comparisons map to the `<ifgreater>`, `<ifgreatereq>`, and `<ifequal>` nodes.
...
Several of the models below reuse the same small sub-expressions. To keep their definitions parallel and concise, those are given here as named functions and called by name in the models that follow. Unlike `open_pbr_anisotropy` (defined after the OpenPBR Surface model), which corresponds to an actual helper node in the data library, these are notation-level abbreviations: the renderable nodegraph expands each one inline.

`ior_to_f0` returns the Schlick facing reflectance (F0), the reflectance at normal incidence, of a dielectric interface from its index of refraction:
...
`coated_emission` attenuates an emission distribution as it passes through a clear-coat: the uncoated emission is tinted by `coat_color` and faded by a Schlick term built from the coat's F0, and this attenuated emission is blended in by `coat_weight`, fading back to the uncoated emission as the coat is removed:
...
This is a surface shading model used in Autodesk products created by the Solid Angle team for the Arnold renderer. It is an über shader built from ten components that are layered and mixed hierarchically[^Georgiev2019].
    ...
    vector3 normal, tangent)
{
    ...
    // closed surfaces use volumetric subsurface scattering. The 'subsurface' weight blends
    // from the diffuse base toward the selected subsurface scattering.
    ...
    // Final surface, with a monochrome alpha-blend opacity from the luminance of the opacity color.
    return surface(
        bsdf = coat_layer,
...
This is a *preview* shading model defined by Pixar for USD[^Pixar2019]. Rather than an über-surface supporting every possible feature, it is an intentionally constrained, basic set of shading controls designed to interchange reliably across film and game pipelines and to be supported natively by the production renderers that ship with USD and Hydra.
...
The `displacement` and `occlusion` inputs are part of the interface but are not implemented by this composition, and so are omitted from its parameter list; `displacement` drives a separate displacement shader, and `occlusion` is applied by the host renderer.

This composition also goes beyond the Schlick reflectance described by the upstream specification in two respects: the metalness workflow blends toward a [&lt;conductor_bsdf>](#node-conductor-bsdf) lobe whose complex index of refraction is derived from `diffuseColor`, rather than remaining a pure Schlick lobe at high `metallic` values; and in transparent mode, a fractional `opacity` is realized as refractive transmission through the surface, where the upstream specification describes only a reduction of the non-specular lighting components in favor of transparency.
    ...
    float emissive_strength, anisotropy_strength, anisotropy_rotation)
{
    ...
    // Tangent rotated about the normal by 'anisotropy_rotation' (radians converted to
    // degrees), used by the base lobes only when a rotation is set.
    vector3 rotated_tangent = normalize(rotate3d(tangent, anisotropy_rotation * -57.295780, normal));
    ...
    vector3 geometry_normal, geometry_coat_normal, geometry_tangent, geometry_coat_tangent)
{
    ...
    // color and biased fore/aft by 'subsurface_scatter_anisotropy', blended in equal measure.
    ...
    // in by 'subsurface_weight'. Thin-walled surfaces use the translucent pair, closed surfaces
    // the volumetric lobe.
    ...
    vector3 absorption_nonneg =
        if (0.0 > absorption_min) { absorption_coeff - vector3(absorption_min) } else { absorption_coeff };
    ...
    // Physical coat darkening: light reflected back down by the coat is partly re-reflected by the
    // base, brightening it, so the base is multiplied by (1 - Kcoat) / (1 - Ebase*Kcoat) to
    // compensate. Kcoat is the coat's internal diffuse reflectance and Ebase an albedo estimate of
    // the base. The effect is modulated by 'coat_weight' and the artistic 'coat_darkening' control.
    ...
    // Final surface: opacity is a direct geometric coverage, and the thin-walled flag is passed
    // through so the renderer can treat the surface as a double-sided shell.
    return surface(
        bsdf = fuzz_layer,
        edf = emission_edf,
...
MaterialX Lama is a modular material layering system developed at Industrial Light & Magic and distributed publicly as part of Pixar's RenderMan[^Pixar2021]. In place of a single über-surface with a fixed set of layers, Lama provides a set of component nodes from which layered materials of arbitrary structure may be composed: the responses `LamaDiffuse`, `LamaTranslucent`, `LamaConductor`, `LamaDielectric`, `LamaGeneralizedSchlick`, `LamaIridescence`, `LamaSheen`, `LamaSSS`, and `LamaEmission`; the combiners `LamaAdd`, `LamaMix`, and `LamaLayer`; and the `LamaSurface` node, which constructs a surface shader from the composed material. Nodegraph implementations of these nodes are provided in the [bxdf/lama](../../libraries/bxdf/lama) folder of the data libraries.
...
[^Pixar2021]: Pixar Animation Studios, **MaterialX Lama**, <https://rmanwiki.pixar.com/display/REN25/MaterialX+Lama>, 2021.
