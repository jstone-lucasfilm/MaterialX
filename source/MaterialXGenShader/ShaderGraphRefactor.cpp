...
//
// ConstantElisionRefactor
//

const string& ConstantElisionRefactor::getName() const
{
    static const string name = "constantElision";
    return name;
}

size_t ConstantElisionRefactor::execute(ShaderGraph& graph, GenContext& context)
{
    size_t numEdits = 0;
...
const string& DotElisionRefactor::getName() const
{
    static const string name = "dotElision";
    return name;
}

size_t DotElisionRefactor::execute(ShaderGraph& graph, GenContext& /*context*/)
{
    size_t numEdits = 0;
    for (ShaderNode* node : graph.getNodes())
    {
        if (node->hasClassification(ShaderNode::Classification::DOT))
        {
...
size_t PremultipliedBsdfAddRefactor::execute(ShaderGraph& graph, GenContext& context)
{
    ...
    for (ShaderNode* mixNode : mixNodes)
    {
        ...
        ShaderOutput* addOutput = addNode->getOutput();
        ...
        for (ShaderInput* downstream : downstreamConnections)
        {
            ...
            downstream->makeConnection(addOutput);
        }
...
size_t DistributeLayerOverMixRefactor::execute(ShaderGraph& graph, GenContext& context)
{
    ...
    // Collect layer nodes to process.
    ...
    for (ShaderNode* layerNode : layerNodes)
    {
        ...
        ShaderOutput* newMixOutput = newMixNode->getOutput();
        ShaderInputVec downstreamConnections = layerOutput->getConnections();
        for (ShaderInput* downstream : downstreamConnections)
        {
            layerOutput->breakConnection(downstream);
            downstream->makeConnection(newMixOutput);
        }
